...
#define __ESP_EVENT_CHAIN_CHECK_POS__(pos)                                  \
  if (pos < 0 || pos > _events.size()) {                                    \
    ESP_LOGE(__ESP_EVENT_CHAIN_DEBUG_TAG__,                                 \
             "Invalid index = %i - numEvents() = %i", pos, _events.size()); \
    panic();                                                                \
  }
...
#define __ESP_EVENT_CHAIN_TRY_CALL__(f)                                       \
  if (!f) {                                                                   \
    ...
    ESP_LOGW(__ESP_EVENT_CHAIN_DEBUG_TAG__, "Couldnt call event at pos = %i", \
             pos);                                                            \
  } else {                                                                    \
    (f)();                                                                    \
  }
...
class EspEventChain {
 ...
 public:
  ...
  EspEventChain(EspEvent e1, Args... events) : _events({e1, events...}) {
    ESP_LOGI(__ESP_EVENT_CHAIN_DEBUG_TAG__, "Constructing chain with %i events",
             sizeof...(events) + 1);
    construct();
  }
   ...
   *
   * @return  The position of the event with getHandle() == handle in the chain
   * if it exists -1 if no match was found
   */
   ...
   * @param handle    The handle to look up, handle != null && handle != "null"
   *
   * @return  A const iterator of the event closest to begin() with getHandle()
  ...
  /**
   * @brief Gets the time required for the entire event chain to complete. Does
   * not account for the time taken by the callbacks
   *
 ...
 private:
   ...
   * follows _currentEventOld in the container _currentEvent =< _currentEventOld
   * if there is no validCurrentEvent() between _currentEventOld and
   ...
   * event
   *
   * post:    _currentEvent method called, _currentEvent == next valid event in
   * chain, ticker armed to call _currentEvent
   *
   */
  void handleTick();
